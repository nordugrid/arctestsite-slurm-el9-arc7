...
branch=$1
cd /
echo  "wget 'https://source.coderefinery.org/nordugrid/arc/-/jobs/artifacts/$branch/download?job=packages_${os_v}' -O artifacts.zip"; 
wget https://source.coderefinery.org/nordugrid/arc/-/jobs/artifacts/$branch/download?job=build_${os_v} -O /artifacts.zip; 
...
cat > /etc/yum.repos.d/nordugrid-testsite.repo <<EOF
[nordugrid-testsite]
...
printf "cat /etc/yum.repos.d/nordugrid-testsite.repo\n"
cat /etc/yum.repos.d/nordugrid-testsite.repo
